 ...
 * 如\S\Cache\Cache::pool('memcache', 'userinfo');
 * 会到config下的memcache配置文件中查找userinfo项
 *
 * @method static get($key)
 * @method static set($key, $value, $ttl = 0)
 *
 *
 * <code>
 * $ret = \S\Cache\Cache::get($key);
 * $ret = \S\Cache\Cache::set($key, $value);
 *
 * //init
 * $mc  = \S\Cache\Cache::pool('memcache', 'userinfo');
 * $mcd = \S\Cache\Cache::pool('memcached', 'userinfo');
 * $apc = \S\Cache\Cache::pool('apc');
 *
 * //set
...
class Cache {
     ...
     *
     * @return \S\Cache\Handler\Abstraction   缓存实例
     * @throws \Exception
     */
    public static function pool($type, $name = '') {
        ...
        if (!isset(self::$pools[$key])) {
            ...
            if (is_subclass_of($instance, $handler_ns . 'Abstraction')) {
                /* @var \S\Cache\Handler\Abstraction $instance */
